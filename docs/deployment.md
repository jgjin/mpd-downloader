...
- A central Temporal cluster to coordinate work for workflow executions
...
1. `temporalio/auto-setup` connects to the database, sets up the database, then runs the server - note auto-setup works well for clusters with one node and doesn't work well for clusters with multiple nodes
...
The cluster data layer security group allows traffic from the cluster compute layer.

The cluster compute layer security group allows traffic to the cluster data layer and to the public internet via HTTPS, not via HTTP. It also allows traffic from the workers to the server and from my local machine to the Temporal web UI.

The worker compute layer security group allows traffic to the cluster compute layer and to the public internet via HTTPS, not via HTTP.
